...
int main(){
	...
	int numb_failed_tests = test_simulation(std::string("RR"),scheduler1,9.5,1.75,15.25);
	resetContainer(ready_Q);

	//SRTF
	Scheduler_SRTF  scheduler2(ready_Q);
	numb_failed_tests = test_simulation(std::string("SRTF"),scheduler2,4.75,4.75,10.5);
	resetContainer(ready_Q);

	//FIFO
	Scheduler_FIFO  scheduler3(ready_Q);
	numb_failed_tests = test_simulation(std::string("FIFO"),scheduler3,8.25,8.25,14);
	resetContainer(ready_Q);
